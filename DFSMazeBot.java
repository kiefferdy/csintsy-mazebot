...
public class DFSMazeBot {
    ...
    public ArrayList<Coordinate> solve(Maze maze) {
    	System.out.println("hi solve() was entered");
        if(!this.explore(maze, maze.getEntrance().getX(), maze.getEntrance().getY(), path)) {
            this.path = null;
            System.out.println("hii null daw yung path");
        }
        return this.path;
    }
    ...
    private boolean explore(Maze maze, int x, int y, ArrayList<Coordinate> path) {
        if(!maze.isValidLocation(x, y) || maze.isExplored(x, y)) {
        	System.out.println("hii invalid daw location");
            return false;
        }
        ...
        //this discards the explored coords that don't lead to the goal
...
//    	Maze temp_m = new Maze(m.getSize());
//    	temp_m = m;
    	solve(m);
    	...
    	for (int i=0; i<n; i++) {
    		...
    		x = coord.getX();
    		y = coord.getY();
    		m.getCoordinate(x, y).setData((char) i);
    	}
